...
import { approveWish, rejectWish } from "@/app/admin/actions";
...
export function PendingWishesAdmin({ wishes }) {
            ...
            {wishes.map((w) => (
              <tr key={w.id} className="align-top">
                ...
                <td className="w-40 shrink-0 px-6 py-8 align-top">
                  <div className="flex flex-col items-stretch justify-start gap-2 sm:items-end">
                    <form action={approveWish.bind(null, w.id)}>
                      <button
                        type="submit"
                        className="w-full min-w-[6.5rem] rounded-xl bg-emerald-600 px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:bg-emerald-700 sm:w-auto"
                      >
                        Approve
                      </button>
                    </form>
                    <form action={rejectWish.bind(null, w.id)}>
                      <button
                        type="submit"
                        className="w-full min-w-[6.5rem] rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 sm:w-auto"
                      >
                        Reject
                      </button>
                    </form>
