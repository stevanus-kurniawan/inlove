...
/**
 * Whether the incoming request is HTTPS (or TLS terminated in front with correct headers).
 * Used so session cookies use Secure only when appropriate — works on http:// and https://
 * without changing env when you add TLS (ensure your reverse proxy sets X-Forwarded-Proto).
 */
function requestIsHttps() {
  try {
    ...
    if (h.get("front-end-https") === "on") return true;
    return false;
  } catch {
    return false;
  }
...
/**
 * Optional override: COOKIE_SECURE=true | false. If unset, follows the request (proxy headers).
 */
